...
`archify`：专业软件架构交互式图表生成，非常适合项目早期，用来做框架与架构设计。在项目从零开始的阶段，你直接向 AI 描述需求、模块划分、组件依赖、数据流向，Archify 会把这套构想转为带 Schema 校验的 JSON 中间表示，再渲染成交互式 HTML 架构图，进而支持自己的分模块开发。
...
| Hashline | 哈希锚点代码编辑 | 专为 AI 代码 Agent 的安全编辑协议：用每行短内容哈希定位改动，无需复述整段旧代码；写入前校验哈希，文件过期则拒绝修改，避免改错位置 |
